...
pub fn find_nearest_location(locations: &[Location]) -> Result<&Location, Box<dyn Error>> {
    ...
    let distance_from_origin = |loc: &Location| {
        (loc.x.powi(2) + loc.y.powi(2) + loc.z.powi(2)) as f64
    };

    locations.iter()
        .filter(|l| l.density() >= 1000.0)
        .min_by(|a, b| {
            distance_from_origin(a).partial_cmp(&distance_from_origin(b)).unwrap()
        })
        .ok_or("No locations found with density >= 1000".into())
}
